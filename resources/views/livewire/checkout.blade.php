        ...
        <section aria-labelledby="payment-and-shipping-heading"
            ...
            <div class="mx-auto max-w-2xl px-4 lg:max-w-none lg:px-0">
                ...
                <div>
                    ...
                    <div class="flex flex-row items-center text-white">
                        ...
                        <nav class="flex mb-4" aria-label="Breadcrumb">
                            <ol class="text-xs inline-flex items-center space-x-1 md:space-x-3">
                                <li @class(['font-bold' => $step === \App\Enums\CheckoutStepsEnum::INFORMATION->value])>
                                    <span>{{ \App\Enums\CheckoutStepsEnum::INFORMATION->getName() }}</span>
                                </li>
                                <li @class(['inline-flex items-center', 'font-bold' => $step === \App\Enums\CheckoutStepsEnum::SHIPPING->value])>
                                    <svg class="w-3 h-3 text-gray-400 mx-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 9 4-4-4-4"/>
                                    </svg>
                                    <span>{{ \App\Enums\CheckoutStepsEnum::SHIPPING->getName() }}</span>
                                </li>
                                <li @class(['inline-flex items-center', 'font-bold' => $step === \App\Enums\CheckoutStepsEnum::PAYMENT->value])>

                                    <svg class="w-3 h-3 text-gray-400 mx-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 9 4-4-4-4"/>
                                    </svg>
                                    <span>{{ \App\Enums\CheckoutStepsEnum::PAYMENT->getName() }}</span>
                                </li>
                            </ol>
                        </nav>
                    ...
                    @if ($step === \App\Enums\CheckoutStepsEnum::INFORMATION->value)
                        <x-checkout.information-form />
                    @elseif ($step === \App\Enums\CheckoutStepsEnum::SHIPPING->value)
                        <x-checkout.shipping-form :user="$user" :address="$address" />
                    @elseif ($step === \App\Enums\CheckoutStepsEnum::PAYMENT->value)
